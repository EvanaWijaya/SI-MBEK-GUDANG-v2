<x-owner-app-layout>
    ...
    <div class="min-h-screen flex flex-col" x-data="{ showWeight: false, showPrice: false, showTotalLivestock: false }">
        ...
        <main class="max-w-6xl mx-auto px-4 py-10 space-y-8 flex-1 w-full"> {{-- Stat Cards --}}
            <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
                ...
                @endphp
                @foreach($cards as $c)
                    <div class="p-5 rounded-xl stat-card {{ $c['bg'] }} hover:shadow-lg" @if(isset($c['clickable']))
                    @click="showTotalLivestock = true" style="cursor: pointer;" @endif>
                        ...
                        @if(isset($c['pct']))
                            <p
                                class="mt-2 text-xs font-medium {{ $c['pct'] >= 0 ? 'text-green-600' : 'text-red-600' }} flex items-center">
                                @if($c['pct'] >= 0)
            ...
            <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {{-- Avg Weight --}}
                <div class="dashboard-section p-6 cursor-pointer hover:bg-orange-50" @click="showWeight = true">
                    <div class="flex items-start">
                        <div class="bg-orange-100 p-3 rounded-lg mr-4">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-brand-orange" fill="none"
                ...
                {{-- Avg Price --}}
                <div class="dashboard-section p-6 cursor-pointer hover:bg-orange-50" @click="showPrice = true">
                    <div class="flex items-start">
                        <div class="bg-orange-100 p-3 rounded-lg mr-4">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-brand-orange" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div x-show="showWeight" class="fixed inset-0 flex items-center justify-center modal-bg z-20"
                style="display: none;" x-transition>
                <div class="bg-white rounded-xl shadow-2xl p-8 modal-content w-full max-w-sm mx-4 sm:mx-auto relative">
                    <button @click="showWeight = false"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="border-t pt-2 flex justify-between">
                            ...
                            <span
                                class="font-bold text-brand-orange">{{ round(($kambingAvgWeight + $dombaAvgWeight) / 2, 1) }}
                                kg</span>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="showPrice" class="fixed inset-0 flex items-center justify-center modal-bg z-20"
                style="display: none;" x-transition>
                <div class="bg-white rounded-xl shadow-2xl p-8 modal-content w-full max-w-sm mx-4 sm:mx-auto relative">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="border-t pt-2 flex justify-between">
                            <span>Total Rata-rata</span>
                            <span
                                class="font-bold text-brand-orange">Rp{{ number_format(($kambingAvgPrice + $dombaAvgPrice) / 2, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            {{-- Modal Total Ternak --}}
            <div x-show="showTotalLivestock" class="fixed inset-0 flex items-center justify-center modal-bg z-20"
                style="display: none;" x-transition>
                <div class="bg-white rounded-xl shadow-2xl p-8 modal-content w-full max-w-sm mx-4 sm:mx-auto relative">
                    <button @click="showTotalLivestock = false"
                        class="absolute top-2 right-2 text-gray-400 hover:text-gray-700">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach($users as $u)
                                <tr class="hover:bg-orange-50">
                                    ...
                                    <td class="p-3 text-center font-semibold text-brand-orange">
                                        {{ $u->kambings_count + $u->dombas_count }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            <section>
                <div class="flex justify-between items-center mb-4">
                    ...
                    <a href="{{ route('owner.penitip') }}"
                        class="text-sm text-brand-orange hover:underline font-medium flex items-center">
                        Lihat semua
