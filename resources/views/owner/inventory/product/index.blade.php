<x-owner-app-layout>
    <div class="p-6 max-w-7xl mx-auto">
        ...
        @php
            ...
            $expiringBatches = $allBatches->filter(
                fn($s) =>
            ...
            $expiredBatches = $allBatches->filter(
                fn($s) =>
                $s->expiration_date && $s->quantity > 0
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            {{-- Di Bawah ROP --}}
            <div
                class="bg-white rounded-xl border {{ $belowRopList->count() > 0 ? 'border-red-200' : 'border-gray-100' }} shadow-sm p-5 flex items-center gap-3">
                <div
                    class="w-10 h-10 {{ $belowRopList->count() > 0 ? 'bg-red-50' : 'bg-gray-50' }} rounded-xl flex items-center justify-center flex-shrink-0">
                    <svg class="w-5 h-5 {{ $belowRopList->count() > 0 ? 'text-red-500' : 'text-gray-400' }}" fill="none"
            ...
            {{-- Hampir Expired --}}
            <div
                class="bg-white rounded-xl border {{ $expiringBatches->count() > 0 ? 'border-yellow-200' : 'border-gray-100' }} shadow-sm p-5 flex items-center gap-3">
                <div
                    class="w-10 h-10 {{ $expiringBatches->count() > 0 ? 'bg-yellow-50' : 'bg-gray-50' }} rounded-xl flex items-center justify-center flex-shrink-0">
                    <svg class="w-5 h-5 {{ $expiringBatches->count() > 0 ? 'text-yellow-500' : 'text-gray-400' }}"
            ...
            {{-- Batch Expired --}}
            <div
                class="bg-white rounded-xl border {{ $expiredBatches->count() > 0 ? 'border-red-100' : 'border-gray-100' }} shadow-sm p-5 flex items-center gap-3">
                <div
                    class="w-10 h-10 {{ $expiredBatches->count() > 0 ? 'bg-red-50' : 'bg-gray-50' }} rounded-xl flex items-center justify-center flex-shrink-0">
                    <svg class="w-5 h-5 {{ $expiredBatches->count() > 0 ? 'text-red-400' : 'text-gray-400' }}"
                        fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div>
                    <p
                        class="text-xs {{ $expiredBatches->count() > 0 ? 'text-red-500' : 'text-gray-400' }} font-medium">
                        Batch Expired</p>
                    <p class="text-2xl font-bold {{ $expiredBatches->count() > 0 ? 'text-red-600' : 'text-gray-800' }}">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 mb-5 flex flex-wrap items-end gap-3">
            ...
            <button id="reset-btn"
                class="px-3 py-2 text-sm text-gray-500 hover:text-gray-800 rounded-lg hover:bg-gray-100 transition-colors">Reset</button>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            <th class="px-5 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">Produk
                            </th>
                            <th
                    ...
                    <tbody id="tbl-body" class="divide-y divide-gray-50">
                        @forelse($products as $prod)
                                                ...
                                                <tr class="hover:bg-gray-50/80 transition-colors {{ $isRop ? 'bg-red-50/20' : '' }}"
                                                    ...
                                                    data-category="{{ $prod->category }}" data-source="{{ $prod->source }}"
                                ...
                                'production' => 'Produksi',
                                'purchase' => 'Pembelian'
                            ][$prod->source] ?? ucfirst($prod->source) }}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td class="px-5 py-4 text-center">
                                                        @if($aJual && $aJual->quantity > 0)
                                                            <span
                                                                class="inline-flex px-2 py-0.5 rounded-md text-xs font-semibold bg-blue-100 text-blue-700">
                                                    ...
                                                    <td class="px-5 py-4 text-center">
                                                        @if($aInt && $aInt->quantity > 0)
                                                            <span
                                                                class="inline-flex px-2 py-0.5 rounded-md text-xs font-semibold bg-purple-100 text-purple-700">
                                                    ...
                                                    <td class="px-5 py-4 text-right">
                                                        <a href="{{ route('owner.inventory.product.show', $prod->id) }}"
            ...
            <p id="no-result" class="hidden py-10 text-center text-sm text-gray-400">Tidak ada produk yang sesuai
                filter.</p>
        </div>
