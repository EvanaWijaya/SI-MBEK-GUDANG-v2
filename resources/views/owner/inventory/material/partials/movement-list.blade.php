                    ...
                    @foreach($movements as $mov)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-5 py-3.5 text-xs text-gray-500 capitalize">
                                @php $sourceLabel = ['PO' => 'Purchase Order', 'purchaseOrder' => 'Pembelian', 'adjustment' => 'Penyesuaian Stok', 'production' => 'Produksi']; @endphp
                            ...
                            <td class="px-5 py-3.5 text-xs text-gray-400">
                                @if($mov->reference_id && in_array($mov->source, ['PO', 'purchaseOrder']))
                                    <a href="{{ route('owner.purchase-orders.show', $mov->reference_id) }}"
                                        class="text-orange-500 hover:underline font-mono">#{{ $mov->reference_id }}</a>
                                @else
                                    -
                                @endif
                            </td>
