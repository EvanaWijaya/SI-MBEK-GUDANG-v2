                                    ...
                                    <tr class="{{ ($order->status === 'settlement' || $order->status === 'capture') && $order->dombas ? 'disabled-product' : '' }}"
                                        ...
                                        <td>
                                            <div class="btn-group flex flex-wrap gap-2">
                                                <!-- Detail Button -->
                                                @if ($order->payment_method === 'manual' || $order->payment_method === 'bank_transfer')
                                                    <a href="{{ route('owner.penjualan.manual-invoice', $order->order_id) }}"
                                                        class="btn btn-view btn-sm">
                                                        <i class="fas fa-file-invoice mr-1"></i> Invoice
                                                    </a>
                                                @else
                                                    <a href="{{ route('owner.penjualan.invoice', $order->order_id) }}"
                                                        class="btn btn-view btn-sm">
                                                        <i class="fas fa-file-invoice mr-1"></i> Invoice
                                                    </a>
                                                @endif
        ...
        <script>
            ...
            function confirmAction() {
                if (currentAction === 'editNotes') {
                    ...
                    fetch(`/owner/orders/${currentOrderId}/notes`, {
                        method: 'POST',
                        headers: {
                    ...
                    fetch(`/owner/orders/${currentOrderId}/status`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
