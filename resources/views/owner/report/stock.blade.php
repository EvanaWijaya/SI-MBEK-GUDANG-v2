<x-owner-app-layout>
    ...
    <div class="p-6 max-w-7xl mx-auto">
        ...
        {{-- ── Filter ── --}}
        <form method="GET" action="{{ route('owner.report.stock') }}"
            ...
            <div class="flex flex-wrap items-end gap-3">
                        ...
                        @foreach($sources as $src)
                            @php
                                $sourceLabel = match ($src) {
                                    'purchase', 'purchaseorder' => 'Pembelian',
                                    'production' => 'Produksi',
                                    'sale' => 'Penjualan',
                                    'adjustment', 'manual_adjustment' => 'Penyesuaian Stok',
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('owner.report.stock') }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                ...
                <p class="text-xs text-gray-400 mt-1">{{ \Carbon\Carbon::parse($startDate)->format('d M') }} –
                                ...
                                <tr class="hover:bg-gray-50/60 transition-colors">
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        ...
                                        <p class="text-xs text-gray-400">
                                            {{ \Carbon\Carbon::parse($m->movement_date)->format('H:i') }}
                                        </p>
                                    </td>
                                    <td class="px-4 py-3">
                                        @if($m->stockable)
                                    ...
                                    <td class="px-4 py-3 text-xs text-gray-500 max-w-[180px] truncate">{{ $m->notes ?? '—' }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
