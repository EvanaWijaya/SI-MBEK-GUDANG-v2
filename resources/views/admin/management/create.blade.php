    ...
    <div class="container mx-auto mt-10 px-4">
        ...
        <form action="{{ route('admin.admins.store') }}" method="POST" novalidate class="bg-white shadow-md rounded-lg p-8 border border-gray-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    {{-- Password Sementara --}}
                    <label class="block font-bold mb-2 {{ $errors->has('password') ? 'text-red-600' : 'text-gray-700' }}">Password Sementara</label>
                    ...
                    <p class="text-xs text-gray-400 mt-1 mb-3">Admin baru akan diminta untuk mengubah password saat login pertama kali.</p>
                    @error('password')
                        <p class="mt-1 text-xs text-red-500 font-semibold">{{ $message }}</p>
                    @enderror
                    
                    {{-- Konfirmasi Password --}}
                    <label class="block font-bold mb-2 {{ $errors->has('password') ? 'text-red-600' : 'text-gray-700' }}">Konfirmasi Password</label>
            ...
            <div class="mt-8 pt-4 border-t border-gray-100 flex items-center">
                <button type="submit" class="bg-brand-orange text-white px-6 py-2.5 rounded font-bold hover:bg-orange-700 shadow transition-colors">Simpan Admin</button>
