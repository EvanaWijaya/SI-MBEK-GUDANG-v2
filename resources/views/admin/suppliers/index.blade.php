<x-admin-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-black leading-tight">{{ __('Admin - Master Supplier') }}</h2>
    </x-slot>
                        ...
                        @foreach ($suppliers as $s)
                            <tr>
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm flex gap-2">
                                    ...
                                    <form action="{{ route('admin.suppliers.destroy', $s->id) }}" method="POST" onsubmit="return confirm('Hapus supplier ini?')">
                                        @csrf @method('DELETE')
