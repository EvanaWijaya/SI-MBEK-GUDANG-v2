<x-admin-app-layout>
    <div class="p-6 max-w-7xl mx-auto">
        ...
        <div class="flex items-start justify-between mb-8">
            ...
            <a href="{{ route('admin.purchase-orders.create') }}" class="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-semibold px-5 py-2.5 rounded-lg shadow transition-colors duration-200">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 mb-6 flex flex-wrap items-end gap-3">
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" id="search-input" placeholder="Nama bahan..." class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-400">
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-50" id="material-tbody">
                        @forelse($materials as $material)
                            ...
                            <tr class="hover:bg-gray-50 transition-colors {{ $belowRop ? 'bg-red-50/20' : '' }}" data-name="{{ strtolower($material->material_name) }}" data-status="{{ $belowRop ? 'reorder_point' : 'aman' }}" data-expiry="{{ $dataExpiry }}">
                                ...
                                <td class="px-5 py-4 text-right">
                                    <a href="{{ route('admin.material.show', $material->id) }}" class="inline-flex items-center gap-1.5 text-orange-600 hover:text-orange-800 font-medium text-xs transition-colors">
                        ...
                        @empty
                            <tr><td colspan="8" class="px-5 py-16 text-center text-gray-400">Belum ada data bahan baku</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div id="no-results" class="hidden px-5 py-12 text-center text-sm text-gray-400">Tidak ada bahan baku yang sesuai filter.</div>
        </div>
    </div>
