        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                <div class="lg:col-span-4 flex flex-col gap-4">
                    <div
                        class="bg-white rounded-xl border {{ $belowRop ? 'border-red-200 bg-red-50/10' : 'border-gray-200' }} shadow-sm p-6 flex-1 flex flex-col justify-center">
                    ...
                    <form method="POST" novalidate action="{{ route('admin.materials.update', $material->id) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="number" step="0.01" name="average_usage"
                                    value="{{ $material->average_usage ?? 0 }}"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-orange-400">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-2">Waktu Estimasi Pesanan Tiba
                                    (Hari)</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                    ...
                    <dl class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <dd class="font-medium text-gray-700">{{ $material->unit }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-400">Harga Rata-rata</dt>
                            <dd class="font-medium text-gray-700">Rp
                                {{ number_format($material->average_price, 0, ',', '.') }}
                            </dd>
                        </div>
                        <div class="flex justify-between">
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                    ...
                    <div class="relative h-4 bg-gray-100 rounded-full overflow-hidden">
                        <div class="h-full rounded-full transition-all duration-500 {{ $belowRop ? 'bg-red-500' : ($pct < 60 ? 'bg-yellow-400' : 'bg-green-500') }}"
                            style="width: {{ $pct }}%"></div>
                        <div class="absolute top-0 bottom-0 w-0.5 bg-orange-500 opacity-70"
                            style="left: {{ $ropPct }}%"></div>
                    </div>
                    <form method="POST" action="{{ route('admin.inventory.material.sync', $material->id) }}"
                        class="mt-4">
                        @csrf <button type="submit"
                            class="w-full flex items-center justify-center gap-2 border border-gray-300 hover:bg-gray-50 text-gray-600 font-medium px-3 py-2 rounded-lg text-xs">Sinkronisasi
                            Stok dari Batch</button>
                    </form>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                ...
                <form method="POST" novalidate action="{{ route('admin.inventory.material.adjust', $material->id) }}"
                    class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                    @csrf
                    <div><label class="block text-xs text-gray-500 mb-1.5">Tipe</label><select name="type" id="adj-type"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-orange-400">
                            <option value="in">➕ Tambah</option>
                            <option value="out">➖ Kurangi</option>
                        </select></div>
                    <div><label class="block text-xs text-gray-500 mb-1.5">Jumlah</label><input type="number"
                    ...
                    <div><label class="block text-xs text-gray-500 mb-1.5">Catatan</label><input type="text"
                            name="notes" class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm"></div>
                    <div><button type="submit"
                            class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2.5 rounded-lg text-sm">Simpan</button>
                    </div>
                </form>
            </div>
