...
<div class="bg-white rounded-xl border border-orange-100 shadow-sm p-5">
            ...
            <form action="{{ route('admin.inventory.product.update-rop', $product->id) }}" method="POST" class="flex items-center gap-2">
                ...
                <div class="relative flex-1">
                    <input type="number" name="reorder_point" value="{{ $product->reorder_point ?? 0 }}" min="0"
                        class="w-full text-2xl font-bold text-orange-500 bg-orange-50/50 border-none rounded-lg p-1 focus:ring-2 focus:ring-orange-200 transition-all">
                </div>
                {{-- Tombol Ceklis (Submit) --}}
                <button type="submit" 
                    class="flex-shrink-0 bg-orange-500 hover:bg-orange-600 text-white p-2 rounded-lg transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-1" 
                    title="Simpan Perubahan ROP">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            @if($reorder_point > 0)
                ...
                <div class="relative h-4 bg-gray-100 rounded-full overflow-hidden mb-1">
                    <div class="h-full rounded-full transition-all duration-500 {{ $barCol }}" style="width: {{ $pct }}%"></div>
            ...
            @if($belowRop)
                <div class="mb-3 flex items-start gap-2 bg-red-50 border border-red-200 text-red-700 px-3 py-2.5 rounded-lg text-xs">
                    ...
                    Stok di bawah stok minimum. {{ $product->source === 'production' ? 'Segera jalankan produksi baru.' : 'Segera buat Pemesanan Bahan.' }}
                </div>
            @else
    ...
    {{-- CTA ROP --}}
    @if($belowRop)
        <div class="bg-orange-50 border border-orange-200 rounded-xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-orange-600 mt-0.5">
                        ...
                        Stok minimum <strong>{{ number_format($product->reorder_point) }}</strong> unit.
                    </p>
                </div>
            </div>
